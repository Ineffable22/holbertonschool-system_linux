...
/**
 * get_method - Validate status code of GET request
 * @path: Path with query string
 * @ram_json: JSON representation
 * @response: Status code
 * @id: Id of the JSON object
 *
 * Return: Body with JSON object
 */
char *get_method(char *path, char ram_json[MAX_SIZE][MAX_SIZE],
		 int *response, int *id)
{
	int find = 0, len = 0, i;
	char *body = NULL;
	      ...
	      char *query2, int *id, char *path)
{
	...
	if (*response == 200)
		body = get_method(path, ram_json, &(*response), &(*id));
	...
	else if (*response == 204)
	{
		if (!(*path))
			return (*response = 404, NULL);
		find = atoi(&path[4]);
		if (strncmp(path, "?id=", 3) || find < 0 || (*ram_json[find]) == 0)
			return (*response = 404, NULL);
		memset(ram_json[find], 0, strlen(ram_json[find]));
	}
	return (body);
}
